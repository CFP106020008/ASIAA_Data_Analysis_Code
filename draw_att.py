...
L = ['Contrast=1','Contrast=25','Contrast=50','Contrast=75','Contrast=100','Contrast=125','Contrast=150']

for i in range(0,len(files_abs)):
    Datas = np.loadtxt(files_abs[i])
    Datas[Datas==0] = 1e-20
    for rows in range(0,np.shape(Datas)[0]):
        if Datas[rows,0]>0.55:
            ROW = rows
            break
    #print(Datas[ROW,0])
    tau = (np.log(Datas[:,2]/Datas[:,1]))/(np.log(Datas[ROW,2]/Datas[ROW,1]))
    if i<0.5*len(files_abs):
        plt.plot(1/Datas[:,0],tau,color[i]+dash,label=str('Accretion: '+L[i]))
    else:
        plt.plot(1/Datas[:,0],tau,color[int(i-0.5*len(files_abs))]+line,label=str('Stellar: '+L[int(i-0.5*len(files_abs))]))
