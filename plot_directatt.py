...
for i in range(0,len(files_abs)):
    ...
    for rows in range(0,np.shape(Datas)[0]):
        if Datas[rows,0]>0.3:
            ROW = rows
            break
    tau = (np.log(Datas[:,3]/Datas[:,1]))/(np.log(Datas[ROW,3]/Datas[ROW,1]))
...
#Line_SMC = ax.scatter(SMC[:,0], SMC[:,2]/1.9, color='gray', marker='*', label='SMC')
#Line_Cal = ax.scatter(1/Calzetti[:,0], Calzetti[:,1], color='gray', marker='x', label='Calzetti')
...
plt.ylim([0,10])
plt.tight_layout()
plt.savefig(str(path+"_direct_Attcur.png"),dpi=300)
#plt.show()
